...
# T030: Import refactored library (Spec 003)
...
def get_block_time(height):
    ...
    block_header_b = Ask_Node(
        ["getblockheader", block_hash_b.decode().strip(), True], False
    )
...
# if date mode search for all the blocks on this day
    ...
    print(
        "\nFinding first blocks on " + datetime_entered.strftime("%b %d, %Y"),
        flush=True,
    )
    print("0%..", end="", flush=True)
...
neighbor_up = output_histogram_bin_counts[
...
neighbor_down = output_histogram_bin_counts[
    left_p001 + best_slide - 1 : right_p001 + best_slide - 1
]
