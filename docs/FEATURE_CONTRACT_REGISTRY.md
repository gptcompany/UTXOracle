...
|------|------|------|------|------|------|------|------|------|
| `live_health_surface` | `/health` | `runtime verified` | `tier_1_execution` | `nautilus_dev`, operators | `:8011` | `computed_inline` | `api.apps.live` | Blocking health corroboration for the live execution surface |
| `fastapi_docs_surface` | `/{docs,redoc,openapi.json}` | `code implemented` | `tier_2_operator` | operators | `:8011` | `computed_inline` | `api.apps.live` | Generated API documentation and schema; not execution-eligible |
...
| `live_chart_surface` | `/api/v1/charts/*` | `runtime verified` | `tier_2_operator` | operators | `:8011` | `hybrid` | `scripts.live.worker` + `api.routes.charts` | Depends on live snapshot store and upstream source health; not execution-eligible |
| `live_chart_page_surface` | `/charts/{chart_id}` | `code implemented` | `tier_2_operator` | operators | `:8011` | `computed_inline` | `api.apps.live` | HTML chart entrypoint only; not execution-eligible |
| `prices_surface` | `/api/prices/*` | `runtime verified` | `tier_2_operator` | operators | `:8011` | `questdb` | `scripts.daily_analysis.py` + `api.routes.questdb` | `:8001` remains active only as a secondary legacy path with migration hint header |
| `metrics_latest_surface` | `/api/metrics/latest` | `runtime verified` | `tier_2_operator` | operators | `:8011` | `questdb` | `scripts.daily_analysis.py` + `scripts.metrics.save_metrics_to_db` + `api.routes.questdb` | `:8001` remains active only as a secondary legacy path with migration hint header |
| `whale_query_surface` | `/api/whale/{transactions,summary,transaction/{txid}}` | `runtime verified` | `tier_2_operator` | operators | `:8011` | `questdb` | `mempool whale monitor` + `address cluster bootstrap` + `api.mempool_whale_endpoints` | Canonical whale query family now serves additive `whale_event.v1` fields; entity enrichment is best-effort and `:8001` remains secondary with migration hint header |
...
| `cost_basis_surface` | `/api/metrics/cost-basis` | `code implemented` | `tier_2_operator` | operators | `:8001` | `duckdb_utxo_lifecycle` | `scripts.metrics.cost_basis` + `api.main` | Implemented but not execution-eligible in the first slice |
| `urpd_features_surface` | `/api/metrics/urpd-features` | `runtime verified` | `tier_2_operator` | operators | `:8011` | `questdb` | `scripts.metrics.urpd_features` + `scripts.metrics.materialize_wave1` + `api.routes.questdb` | Scalar features from cost-basis distribution (spec-060); `top_bucket_concentration` is single-bucket, not cluster; historical backtest loading is wired via `MetricLoader` |
...
| `entity_intelligence_surface` | `/api/entities/*` | `code implemented` | `tier_2_operator` | operators | `:8011` | `questdb` | `scripts.clustering.backfill_entity_registry_sampled` + `scripts.live.flow_aggregator` + `scripts.bootstrap.sync_entities_to_questdb` + `api.routes.entities` | Operator-only entity intelligence surface on `:8011`; accepts legacy `cluster:*` aliases read-only and depends on daily registry/flow materialization |
...
| `wallet_and_cohort_surface` | `/api/metrics/{address-cohorts,wallet-waves,absorption-rates}` | `runtime verified` | `tier_2_operator` | operators | `:8011` | `questdb` | `scripts.metrics.materialize_wave1` + `api.routes.questdb` | Latest snapshots are materialized and served on `:8011`; `wallet-waves/history` remains outside the admitted slice |
...
| `feature_meta_surface` | `/api/meta/features` | `code implemented` | `tier_2_operator` | operators | `:8011` | `computed_inline` | `api.routes.meta` | Serves the feature provenance manifest; not execution-eligible |
