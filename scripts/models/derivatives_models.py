"""
Data Models for Derivatives Historical Integration (spec-008).

Defines dataclasses for:
- FundingRateSignal: Funding rate with contrarian vote
- OpenInterestSignal: OI with context-aware vote
- EnhancedFusionResult: 4-component Monte Carlo fusion result
...
class EnhancedFusionResult:
    ...
    def __post_init__(self):
        ...
        # Validate weights sum to ~1.0 for active components
...
class BacktestResult:
    ...
    def __post_init__(self):
        ...
        if (
            self.total_signals
            != self.buy_signals + self.sell_signals + self.hold_signals
        ):
