# Data Model: ResearchBitcoin.net API Integration

**Phase 1 Design Output** | **Date**: 2025-12-26 | **Spec**: [spec.md](./spec.md)

## Overview

Pydantic models for RBN API integration, covering fetcher requests/responses, metric mapping, comparison results, and validation reports.
...
RBNConfig (settings)
    ├── RBNFetcher (service)
    │   ├── RBNMetricRequest → RBNMetricResponse
    │   └── Cache management
    │
    └── ValidationService (service)
        ├── MetricComparison (per date)
        └── ValidationReport (aggregate)

RBNMetricMapping (static registry)
    └── Maps UTXOracle specs → RBN endpoints
```
...
class RBNMetricRequest(BaseModel):
    """Request parameters for fetching a metric."""
...
class MetricComparison(BaseModel):
    ...
    def create(
        ...
        rel_diff = (abs_diff / abs(rbn_val)) * 100 if rbn_val != 0 else float('inf')
...
class ValidationReport(BaseModel):
    ...
    def from_comparisons(
        ...
        valid_diffs = [c.relative_diff_pct for c in comparisons if c.relative_diff_pct is not None]
...
| Field | Validation |
|-------|------------|
| `token` | UUID format, non-empty |
| `start_date` | Not in future, max 1 year ago (Tier 0) |
| `end_date` | >= start_date, <= today |
| `tolerance_pct` | 0.0 < x <= 100.0 |
| `metric_id` | Must exist in RBN_METRICS registry |
...
RBNMetricRequest
    ↓ (fetch with token)
RBNMetricResponse [cached=False]
    ↓ (store in cache)
RBNMetricResponse [cached=True] (subsequent requests)
