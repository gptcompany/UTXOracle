...
| `cluster_confidence` | DOUBLE | Confidence that the cluster artifact itself is stable enough to support attribution |
...
| `first_seen` | TIMESTAMP | Earliest evidence for this mapping |
| `last_seen` | TIMESTAMP | Latest evidence update for this mapping |
...
`label_kind` captures presentation role only. Whether a label was inherited, curated, or externally sourced belongs in provenance, not in `label_kind`.
...
| `entity_id` | STRING | |
| `label` | STRING | |
| `source_kind` | ENUM | heuristic, curated_csv, manual_operator, external_dataset, inherited_cluster_label |
...
The overall confidence is the minimum of the participating component confidences, so it never exceeds the weakest materially relevant input.

`confidence_overall = min(participating component confidences)`
...
Rules:
- When all three are present, `confidence_overall = min(cluster_confidence, mapping_confidence, label_confidence)`.
- When a component is absent, the API must declare which components participated rather than fabricating completeness.
- First-slice v1 does not use multiplicative or weighted composition.
...
- `entity_registry_serving`: Collapsed view of registry + primary labels + component confidence fields required by metadata routes.
- `entity_provenance_serving`: Collapsed JSON summary of evidence and review state for API exposure.
