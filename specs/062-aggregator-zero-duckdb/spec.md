...
- What happens during a partial mid-run failure (e.g., `save_mvrv_daily` succeeds but `save_nupl_daily` raises)? Per FR-007, sibling-row writes are NOT rolled back. The successful row stays. The failed row is logged at ERROR (FR-011), the Discord webhook is posted (FR-012), and the next idempotent re-run upserts both rows via `DEDUP UPSERT KEYS(ts)`. Reads of `/v1/streams/health` will show the failing stream as STALE until the next successful run.
...
- **FR-007**: Each daily row (`mvrv_daily`, `nupl_daily`, `realized_cap_daily`) is persisted independently. A failure to write one row MUST log an ERROR and MUST NOT roll back successful sibling-row writes. Rationale (strangler-fig R5): the dual-write design preserves any successful QuestDB row even when a sibling save fails — the failed row will be reproduced on the next idempotent re-run. Per-row idempotency is provided by QuestDB `DEDUP UPSERT KEYS(ts)` (FR-004); cross-row atomicity is explicitly NOT a requirement and would defeat the strangler-fig fallback semantics.
