...
- **FR-005**: The producer MUST expose a rollback configuration toggle (env var or CLI flag, owner-decided in `/speckit.plan`) that disables the QuestDB write half without touching the DuckDB write. With the toggle OFF, `aggregate_flows()` MUST NOT open any QuestDB connection.
...
- **FR-007**: A QuestDB write failure MUST post a one-line Discord webhook notification (re-using the spec-062 FR-012 surface) with `entity_flows_daily` as the stream identifier and the failing date. Successful runs MUST NOT post.
...
- **FR-010**: Schema parity: the QuestDB `entity_flows_daily` columns and types MUST be a 1:1 mirror of the DuckDB columns as observed at the start of spec-063 (discovered in `data-model.md`). Any deliberate divergence MUST be documented in `decisions.md`.
